param(
  [string]$SourceRoot = "",
  [string]$OutputRoot = "public/uav-gallery",
  [string]$ManifestFile = "src/data/uavGalleryManifest.ts",
  [string]$FfmpegPath = "",
  [string]$FfprobePath = "",
  [int]$ThumbWidth = 520,
...
if (-not $SourceRoot) {
  ...
  $uavFolderName = -join (@(0x5168, 0x673A, 0x578B, 0x65E0, 0x4EBA, 0x673A, 0x56FE, 0x5E93) | ForEach-Object { [char]$_ })
  $SourceRoot = Join-Path (Join-Path $projectRoot $resourcePackageName) $uavFolderName
}
...
$uavGalleryZh = -join (@(0x65E0, 0x4EBA, 0x673A, 0x56FE, 0x5E93) | ForEach-Object { [char]$_ })
...
if ($ignoredVideos.Count -gt 0) {
  Write-Host "Ignoring $($ignoredVideos.Count) video file(s) in UAV gallery source folder."
}
...
foreach ($image in $images) {
  ...
  Write-WebpVariant -SourcePath $image.FullName -OutputPath $thumbFile -MaxWidth $ThumbWidth -Quality 62
  Write-WebpVariant -SourcePath $image.FullName -OutputPath $displayFile -MaxWidth $DisplayWidth -Quality 70
  Write-WebpVariant -SourcePath $image.FullName -OutputPath $fullFile -MaxWidth $FullWidth -Quality 78

  $safeTitle = Get-SafeTitle -FileName $image.Name
  $items.Add([pscustomobject]@{
    id = $id
    title = @{
      zh = "$uavGalleryZh {0:D2}" -f $index
      en = "UAV gallery {0:D2}" -f $index
    }
...
const rawUavGalleryImages = $json as const;
...
Write-Host "Generated $($items.Count) UAV gallery image entries in $ManifestFile"
